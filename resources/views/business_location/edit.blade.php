    ...
    <form autocomplete="off" action="{{ route('locations.update', ['location' => $location->id]) }}" method="POST" class="submit-location">
        ...
        <main class="px-4 flex flex-col gap-4 xs/max:gap-3 mb-8">
            <section class="flex flex-col gap-1">
                ...
                {!! FormCustom::togglebutton('status', $location->status, ['Active', 'Inactive'], []) !!}
            </section>
            <section class="flex flex-col gap-1">
                <label class="font-normal text-sm text-gray-500 xs/max:text-xs">name*</label>
                {!! FormCustom::input('name', $location->name, [ "placeholder" => 'Enter new your name']) !!}
            </section>
            <section class="flex flex-col gap-1">
                <label class="font-normal text-sm text-gray-500 xs/max:text-xs">website</label>
                {!! FormCustom::input('website', $location->website, [ "placeholder" => 'Enter new your website']) !!}
            </section>
            <section class="flex flex-col gap-1">
            ...
            <section class="flex flex-col gap-1">
                ...
                {!! FormCustom::input('country', $location->country, [ "placeholder" => 'Enter new your country']) !!}
            </section>
            <section class="flex flex-col gap-1">
                <label class="font-normal text-sm text-gray-500 xs/max:text-xs">state</label>
                {!! FormCustom::input('state', $location->state, [ "placeholder" => 'Enter new your state']) !!}
            </section>
            <section class="flex flex-col gap-1">
                <label class="font-normal text-sm text-gray-500 xs/max:text-xs">city</label>
                {!! FormCustom::input('city', $location->city, [ "placeholder" => 'Enter new your city']) !!}
            </section>
            <section class="flex flex-col gap-1">
