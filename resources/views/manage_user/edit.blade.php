<main class="flex flex-col gap-8  pt-4 w-[375px]">
    ...
    <form autocomplete="off" action="{{ route('users.update', ['user' => $user->id]) }}" method="POST"
        class="submit-user">
        @csrf
        <!-- {{ csrf_field() }} -->
        <main class="px-4 flex flex-col gap-4 xs/max:gap-3 mb-8">
            ...
            <div class='flex justify-between items-start'>
                <div class="relative">
                    <button type="button" id="remove-img" class="hidden absolute right-0 bg-red-500 rounded-full text-white p-0.5"
                    ...
                    <div class="h-16 w-16 bg-gray-50 rounded-full overflow-hidden">
                        <img src="{{ asset('storage/profiles/'.$user->photo.'')}}" class="object-contain w-full h-full" id="photo_preview">
                    </div>
                </div>
            ...
            <section class="flex flex-col gap-1">
                ...
                {!! FormCustom::input('first_name', $user->first_name, [ "placeholder" => 'Enter new your first_name'])
                !!}
            </section>
            <section class="flex flex-col gap-1">
