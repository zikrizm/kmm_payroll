...
@section('content')
    <div class="h-full flex-1 pb-12 px-8 xs/max:p-4 xs/max:pb-8 overflow-y-auto overflow-x-hidden relative">
        <main class="flex flex-col gap-2 xs/max:gap-4 h-full">
            <header class="min-h-[80px] w-full flex justify-between items-center">
                <p class="font-semibold text-2xl text-gray-700 xs/max:text-xl">Break time</p>
    ...
    <x-modal-confirmation classSubmit="submit-delete-break-time"></x-modal-confirmation>
    ...
    async function onInit(page, q = '') {
        ...
        // * get table ----->
        // *
        var res = await Utils.table('/break-time?'+(new URLSearchParams({ page, q}).toString()), null);
    ...
    async function get_modal(idBreakTime) {
        // **
        // * open modal form ----->
        // *
        var URL = (idBreakTime) ? '/break-time/' + idBreakTime + '/edit' : '/break-time/create';
        var res = await Utils.modal(URL, null);
        var toggle = new Toggle();
        var toggleStatus = toggle.button('status', (!idBreakTime) ? 'active': '', {})
    ...
    function open_modal_confirm(idBreakTime) {
        // **
        // * open modal confirm ----->
        // *
        Utils.modal_confirm('.submit-delete-break-time', '/break-time/' + idBreakTime, null, () => {
            onInit();
        })
    }
</script>
