<form autocomplete="off" action="{{ route('resign.update', ['resign' => $resign['id']]) }}" method="POST"
        ...
        <header class="px-4 flex flex-col gap-5 pt-4 xs/max:gap-3 relative">
            ...
            <div class="flex flex-col gap-1">
                <div class="flex items-start gap-2">
                    ...
                    <div>
                        <p class="text-xl font-semibold text-gray-900">Pengunduran karyawan
                        </p>
        ...
        <div>
            <main class="px-4 flex flex-col gap-2.5 xs/max:gap-3 mb-8">
                <section class="flex flex-col gap-1">
                    ...
                    {!! FormCustom::input('employee', '('.$resign['employee']['emp_code'].') '.$resign['employee']['first_name'],
                ...
                <section class="flex flex-col gap-1">
                    ...
                    {!! FormCustom::input('resign_date', $resign['resign_date'], [ 'placeholder' => 'Pilih tanggal
                    Pengunduran Diri
                    karyawan',
                ...
                <div class="flex items-start gap-4 w-full">
                    <section class="flex flex-col gap-1 flex-1 w-1/2">
                        ...
                        <select class="select2" name="resign_type">
                            <option value="1" {{ $resign['resign_type']==1? 'selected' : '' }}>Berhenti</option>
                            <option value="2" {{ $resign['resign_type']==2? 'selected' : '' }}>Dihentikan</option>
                            <option value="3" {{ $resign['resign_type']==3? 'selected' : '' }}>Mengundurkan diri
                            </option>
                            <option value="4" {{ $resign['resign_type']==4? 'selected' : '' }}>Transfer</option>
                            <option value="5" {{ $resign['resign_type']==5? 'selected' : '' }}>Mempertahankan pekerjaan
                                tanpa bayaran</option>
                        </select>
                    ...
                    <section class="flex flex-col gap-1 flex-1 w-1/2">
                        ...
                        <select class="select2" name="disableatt">
                            <option value="False" {{ !$resign['disableatt']? 'selected' : '' }}>Enable</option>
                            <option value="True" {{ $resign['disableatt']? 'selected' : '' }}>Disable</option>
                        </select>
                        <label class="font-normal text-xs text-red-500 xs/max:text-xs gender hint-text"></label>
                    </section>
                </div>
