<form autocomplete="off" action="{{ route('transaction.store') }}" method="POST"
    class="submit-transaction flex items-start gap-5 justify-center">
    @csrf
    <!-- {{ csrf_field() }} -->
    <section
        class="flex flex-col gap-8 pt-4 bg-white w-[375px] max-h-[90vh] overflow-y-auto overflow-x-hidden relative rounded-lg no-scrollbar">
        <header class="px-4 flex flex-col gap-5 pt-4 xs/max:gap-3 relative">
            ...
            <div class="flex flex-col gap-1">
                <div class="flex items-start gap-2">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-gray-900 xs/max:text-xl xs/max:font-semibold">New transaction
                        </p>
