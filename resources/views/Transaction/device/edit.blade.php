<form autocomplete="off" action="{{ route('device.update', ['device' => $device['id']]) }}" method="POST"
    ...
    <!-- {{ csrf_field() }} -->
    <main
        class="flex flex-col gap-8  pt-4 w-[375px] bg-white max-h-[95vh] overflow-y-auto overflow-x-hidden no-scrollbar relative rounded-lg">
            ...
            <main class="px-4 flex flex-col gap-4 xs/max:gap-3 mb-8">
                ...
                <section class="flex flex-col gap-1">
                    ...
                    <select class="select2" name="punch_type">
                        <option value="check-in-out" {{ $device['punch_type']=='check-in-out' ? 'selected' : '' }}>Check In Out</option>
                        <option value="check-in" {{ $device['punch_type']=='check-in' ? 'selected' : '' }}>Check In</option>
                        <option value="check-out" {{ $device['punch_type']=='check-out' ? 'selected' : '' }}>Check Out</option>
                    </select>
