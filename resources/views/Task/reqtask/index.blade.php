...
<div class="flex flex-col gap-6 flex-1 h-full overflow-auto bg-white px-8 pt-8 pb-12">
    <header class="flex justify-between items-start">
        <div class="flex flex-col gap-1">
            <p class="text-3xl font-medium text-gray-900">Rendaman</p>
            <p class="text-base font-normal text-gray-500">Disini untuk mengatur rendaman.
            </p>
        </div>
        <div class="">
    ...
    <div class="flex justify-between">
        <div class="w-72">
            ...
            'placeholder' => 'Pilih tanggal rendaman',
            'class' => 'date_input',
            'readonly' => true,
            'prefixiconname' => 'calendar',
            ]) !!}
        </div>
        <x-ui.search-data placeholder="Cari rendaman" url="{{ route('rendaman.index') }}" />
    </div>
    <div class="table-content"></div>
    <x-ui.confirm-modal class="submit-delete-rendaman"></x-ui.confirm-modal>
</div>
...
<script type="application/javascript">
    let dataParams = {};
        ...
        async function onInit(data) {
            ...
            // * get table ----->
            // *
            var res = await ApiService.get_table('/rendaman', data);
        ...
        async function get_modal(rendaman_id) {
            // **
            // * open modal form ----->
            // *
            var URL = (rendaman_id) ? '/rendaman/' + rendaman_id + '/edit' : '/rendaman/create';
            ...
            //             if ($('#rendaman-content').is(':hidden')) {
            //                 $('#rendaman-content').toggle('hidden');
            //             }
            //             if($('.rendaman-date').length) {
            //                 $('.rendaman-date').data('daterangepicker').startDate = moment(select_operational.start_date);
            //                 $('.rendaman-date').data('daterangepicker').endDate = moment(select_operational.end_date);
            //                 $('.rendaman-date').data('daterangepicker').minDate = moment(select_operational.start_date);
            //                 $('.rendaman-date').data('daterangepicker').maxDate = moment(select_operational.end_date);
            //             }
            //         }
            //     } catch (error) { }
            // });
            ...
            // * submit form ----->
            // *
            var resSubmit = ApiService.submit_form('.submit-rendaman', (data) => { 
                onInit( { q: $('.search-data-input').val() });
            });
        }

        async function open_modal_confirm(rendaman_id) {
        // **
        // * open modal confirm ----->
        // *
        await ApiService.get_confirm('.submit-delete-rendaman', '/rendaman/' + rendaman_id, null, () => {
            onInit();
        })
    }
</script>
