                    ...
                    <div class="flex-1 flex flex-col border-r last:border-0 py-1 min-w-[170px]">
                        @forelse(($request_tasks[$item] ?? []) as $key => $item_task)
                            <div class="group px-2 py-1 h-[65px]">
                                <div
                                    class="h-full border rounded flex justify-between flex-col p-2 relative hover:bg-gray-50 ">
                                    <p class="text-gray-700 font-medium text-sm truncate w-[135px] break-words">Tugas -
                                        {{ !empty($item_task['position']) ? $item_task->position->position_name ?? '' : '-' }}
                                    </p>
                                    <div class="flex justify-start items-center relative h-[18px] gap-0.5">
                                        @foreach ($item_task->request_task_has_emps as $key => $emp)
                                            <div
                                                class='rounded-full border-white bg-white border-[1.5px] h-[18px] w-[18px] overflow-hidden'>
                                                @if (!empty($emp->employee->photo))
                                                    <img src="{{ config('constants.api_zkteco') }}{{ $emp->employee->photo }}"
                                    ...
                                    <div
                                        ...
                                        <button type="button"
                                            onclick="open_modal_confirm_task(this,'{{ $item_task->id }}')"
                        ...
                        @forelse(($operationals[$item] ?? []) as $key => $item_op)
                            <div class="group px-2 py-1 h-[65px]">
                                <div
                                    class="h-full border rounded flex justify-between flex-col p-2 relative hover:bg-gray-50 ">
                                    <p class="text-gray-700 font-medium text-sm truncate w-[135px] break-words">
                                        {{ $item_op->department->dept_name }}</p>
                                    <div class="flex items-center gap-0.5 flex-1" style="flex-wrap: wrap;">
                                        @forelse ($item_op->operational_has_timetables->where('status', 'active') as $item_timetable)
                                            <div
                                                ...
                                                <div
                                                    ...
                                                    <div
                                                        class="flex w-full items-center gap-1.5 text-gray-400 justify-start">
                                                        <x-icon icon="clock" width=12 height=12 viewBox="20 20" />
                                                        <div
                                                            class="flex items-center gap-1 text-gray-400 text-[10px] mt-0.5">
                                                            <p class="truncate">
                                        ...
                                        @empty
                                            <p class="text-[10px] text-gray-500 italic">Tidak ada yang aktif</p>
                                        @endforelse
                                    </div>
                                    <div
                                        class="absolute hidden group-hover:flex flex-col items-center justify-center gap-0.5 top-0.5 right-0.5">
                                        <button type="button"
                                            onclick="get_modal('{{ $item_op->id }}', {date: '{{ $item }}'})"
                                            class="bg-white text-violet-500 border rounded border-violet-100 p-1 hover:bg-violet-100">
                                            <x-icon icon="edit" width=12 height=12 viewBox="20 20" />
                                        </button>
