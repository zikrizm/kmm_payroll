...
@section('content')
    ...
    let dataParams = {};
        ...
        async function get_modal(id, data_operasional) {
            ...
            var anElement = new AutoNumeric.multiple('.number',{unformatOnSubmit: true,decimalCharacterAlternative:'.',modifyValueOnWheel: false, minimumValue: 0,decimalCharacter: ',', digitGroupSeparator : ""});
            ...
            if(!data_operasional) {
                $('.operational_date').daterangepicker({
                    ...
                    var dept_val = $('.select2-department').val();
                    if(dept_val) {
                         if(Array.isArray(dept_val) && dept_val.length > 0) dept_val = dept_val[0];
                        var date = start.format('YYYY-MM-DD')+' - '+ end.format('YYYY-MM-DD');
                        if(dept_val) get_card_operational(true, date, dept_val);
                    }
                });
            }
            ...
            $('.select2-department').on('change', async function (e) {
                ...
                if(val) get_card_operational((!data_operasional), date, val);
            });
