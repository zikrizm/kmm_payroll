<form autocomplete="off" action="{{ route('operasional.add-employees-to-help.store') }}" method="POST"
    class="submit-add-employees-to-help flex items-start gap-5 justify-center">
    @csrf
    <!-- {{ csrf_field() }} -->
    <section
            ...
            <div class="flex flex-col gap-1">
                <div class="flex items-start gap-2">
                    <div
                        ...
                        <x-icon icon="user-plus" width=16 height=16 viewBox="20 20" />
                    </div>
                    <div>
                        <p class="text-xl font-semibold text-gray-900">Tambah bantuan
                        </p>
                        <p class="text-sm font-normal text-gray-500 xs/max:text-xs">
                            Harap pilih karyawan untuk membantu bagian lain.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div>
            <main class="px-4 flex flex-col gap-2.5 xs/max:gap-3 mb-8">
                <section class="flex flex-col gap-1 flex-1">
                    ...
                    'placeholder' => 'Pilih tanggal membantu',
                    'class' => 'employee-helpdate',
                    'readonly' => true,
                    'prefixiconname' => 'calendar',
                    ]) !!}
                </section>
                <section class="flex flex-col gap-1 flex-2">
                    <label class="font-normal text-sm text-gray-500 xs/max:text-xs">Sub-bagian <span
                            class="font-bold text-xs">({{ $operational->dept_name }})</span>*</label>
                    <select class="select2" name="department">
                        <option value="" disabled selected>Silahkan Pilih</option>
                        @foreach ($operational->operational_group as $item)
                ...
                <section class="flex flex-col gap-1">
                    ...
                    <select class="select2-employee" name="emp_id[]" multiple="multiple">
                    </select>
