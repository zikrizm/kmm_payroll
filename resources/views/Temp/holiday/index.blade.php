...
<div class="h-full flex-1 pb-12 px-8 xs/max:p-4 xs/max:pb-8 overflow-y-auto overflow-x-hidden relative">
    <main class="flex flex-col gap-2 xs/max:gap-4 h-full">
        <header class="min-h-[80px] w-full flex justify-between items-center">
            <p class="font-semibold text-2xl text-gray-700 xs/max:text-xl">Holiday management</p>
...
<x-modal-confirmation classSubmit="submit-delete-holiday"></x-modal-confirmation>
...
<script type="application/javascript">
    ...
    async function onInit(page, q = '') {
        ...
        // * get table ----->
        // *
        var res = await Utils.table('/holidays?'+(new URLSearchParams({ page, q}).toString()), null);
    ...
    async function get_modal(idHoliday) {
        // **
        // * open modal form ----->
        // *
        ...
        $('input[name="start_date"]').daterangepicker({
            ...
            startDate: (stV != '') ? stV : moment().startOf('hour'),
            endDate: (stV != '') ? stV : moment().startOf('hour').add(32, 'hour'),
            drops: "auto",
            locale: {
        ...
        $('input[name="end_date"]').daterangepicker({
            ...
            timePicker24Hour: true,
            timePicker: true,
            startDate: (etV != '') ? etV : moment().startOf('hour'),
            endDate: (etV != '') ? etV : moment().startOf('hour').add(32, 'hour'),
            drops: "auto",
            locale: {
                format: 'YYYY-MM-DD HH:mm:ss'
            }
        ...
        // * submit form ----->
        // *
        var resSubmit = Utils.submit('.submit-holiday', (data) => { 
            onInit();
        });
    }

    function open_modal_confirm(idHoliday) {
        // **
        // * open modal confirm ----->
        // *
        Utils.modal_confirm('.submit-delete-holiday', '/holidays/' + idHoliday, null, () => {
            onInit();
        })
    }
</script>
