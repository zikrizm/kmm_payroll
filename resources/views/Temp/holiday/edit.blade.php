    ...
    <form autocomplete="off" action="{{ route('holidays.update', ['holiday' => $holiday->id]) }}" method="POST"
        ...
        <main class="px-4 flex flex-col gap-4 xs/max:gap-3 mb-8">
            <section class="flex flex-col gap-1">
                ...
                {!! FormCustom::input('name', $holiday->name, [ "placeholder" => 'Enter new your name']) !!}
            </section>
            <div class="flex flex-col gap-1">
            ...
            <section class="flex flex-col gap-1">
                ...
                {!! FormCustom::togglebutton('status', $holiday->status, ['Active', 'Inactive'], []) !!}
            </section>
            <div class="flex flex-col gap-1">
                <label class="font-normal text-sm text-gray-500 xs/max:text-xs">Notes*</label>
                {!! FormCustom::textarea('notes', $holiday->notes, [ "placeholder" => 'Enter new your notes'])
                !!}
            </div>
        </main>
        <hr>
