<form autocomplete="off" action="{{ route('area.update', ['area' => $area['id']]) }}" method="POST" class="submit-area">
    @csrf
    <!-- {{ csrf_field() }} -->
    <main
        ...
        <div>
            <main class="px-4 flex flex-col gap-4 xs/max:gap-3 mb-8">
                <section class="flex flex-col gap-1">
                    <label class="font-normal text-sm text-gray-500 xs/max:text-xs">Area code*</label>
                    {!! FormCustom::input('area_code', $area['area_code'], [ "placeholder" => 'Enter new your
                    area code']) !!}
                </section>
                <section class="flex flex-col gap-1">
                    <label class="font-normal text-sm text-gray-500 xs/max:text-xs">Area name*</label>
                    {!! FormCustom::input('area_name', $area['area_name'], [ "placeholder" => 'Enter new your
                    area name']) !!}
                </section>
                <section class="flex flex-col gap-1">
                    <label class="font-normal text-sm text-gray-500 xs/max:text-xs">Parent area*</label>
                    <select class="select2" name="parent_area">
                        <option value="" selected>Silahkan Pilih</option>
                        @foreach ($areas['data'] as $item)
                        <option value="{{ $item['id'] }}" {{ (!empty($area['parent_area']) &&
                            ($area['parent_area']['id']==$item['id'])) ? 'selected' : '' }}>{{
                            $item['area_name'] }}</option>
                        @endforeach
                    </select>
                    <label class="font-normal text-xs text-red-500 xs/max:text-xs parent_area text-error"></label>
                </section>
            </main>
            <hr>
