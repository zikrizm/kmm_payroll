<main class="flex flex-col gap-8  pt-4 w-[375px] xs/max:w-[280px]">
    <header class="px-4 flex flex-col gap-5 pt-4 xs/max:gap-3 relative">
        ...
        <div class="flex flex-col gap-1">
            <div class="flex items-start gap-2">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900 xs/max:text-xl xs/max:font-semibold">New area</p>
    ...
    <form autocomplete="off" action="{{ route('area.store') }}" method="POST" class="submit-area">
        @csrf
        <!-- {{ csrf_field() }} -->
        <main class="px-4 flex flex-col gap-4 xs/max:gap-3 mb-8">
            <section class="flex flex-col gap-1">
                ...
                {!! FormCustom::input('area_code', null, [ "placeholder" => 'Enter new your area code']) !!}
            </section>
            <section class="flex flex-col gap-1">
                <label class="font-normal text-sm text-gray-500 xs/max:text-xs">Area name*</label>
                {!! FormCustom::input('area_name', null, [ "placeholder" => 'Enter new your area name']) !!}
            </section>
            <section class="flex flex-col gap-1">
