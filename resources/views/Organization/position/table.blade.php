    ...
    <table class='table border-collapse w-full'>
        <thead class='border-b border-gray-200 bg-gray-50'>
            <tr class=''>
                <th class='text-left'>
                    <div class='flex items-center'>
                        ...
                        <div class='px-6 py-3 cursor-pointer flex-1'>
                            <x-ui.sort-table text="Kode jabatan" url="{{ route('position.index') }}" field="position_code"
                                order="{{ $order }}" />
                        </div>
                    </div>
                </th>
                ...
                <th class='px-3 py-3 text-left cursor-pointer'>
                    <p class="text-xs font-medium text-gray-500 truncate cursor-pointer">Istirahat Tambahan (Menit)</p>
                </th>
                <th class='px-3 py-3 text-left cursor-pointer'>
                ...
                <tr class='hover:bg-gray-50 border-b border-gray-200'>
                    ...
                    <td class='px-3 py text-gray-500 text-sm'>
                        <div class="flex justify-center">
                            @if (!empty($item['must_attend']) && $item['must_attend'])
                                <span
                                    class="w-4 h-4 rounded-full bg-green-600 flex items-center justify-center text-white">
                                    <x-icon icon="check" width=12 height=12 viewBox="20 20" />
                                </span>
                            @else
                                <span
                                    class="w-4 h-4 rounded-full bg-gray-300 flex items-center justify-center text-white">
                                    <x-icon icon="check" width=12 height=12 viewBox="20 20" />
                                </span>
                            @endif
                        </div>
                    ...
                    <td class='px-3 py text-gray-500 text-sm'>
                        <div class="flex items-center gap-2">
                            @if (!empty($item['extra_break_time']))
                            {{ $item['extra_break_time'] ?? '-' }} Menit
                            @else
                            -
                            @endif
                            @if (!empty($item['enable_extra_break_time']) && $item['enable_extra_break_time'])
                                <span
                                    class="w-4 h-4 rounded-full bg-green-600 flex items-center justify-center text-white">
                                    <x-icon icon="check" width=12 height=12 viewBox="20 20" />
                                </span>
