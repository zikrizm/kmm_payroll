...
<x-modal-confirmation classSubmit='submit-delete-role'>
...
<script type="application/javascript">
    ...
    async function onInit() {
        var res = await Utils.table('/access-control', null);
        $('.table-content').html(res);
    }

    async function getModal(idRole) {
        var URL = (idRole) ? '/access-control/' + idRole + '/edit' : '/access-control/create';
        var res = await Utils.modal(URL, null);
        var resSubmit = Utils.submit('.submit-role', (data) => {
            onInit();
         });
    }
    async function openModalConfirm(idRole) {
        Utils.modal_confirm('submit-delete-role', '/access-control/' + idRole, () => {
            onInit();
        })
    }
</script>
