<main class="flex flex-col gap-8  pt-4 w-[375px] xs/max:w-[280px]">
    ...
    <form autocomplete="off" action="{{ route('work-sections.store') }}" method="POST" class="submit-work-section">
        @csrf
        <!-- {{ csrf_field() }} -->
        <main class="px-4 flex flex-col gap-4 xs/max:gap-3 mb-8">
            <section class="flex flex-col gap-1">
                ...
                {!! FormCustom::input('name', null, [ "placeholder" => 'Enter new your section name']) !!}
            </section>
            <section class="flex flex-col gap-1">
            ...
            <section class="flex flex-col gap-5">
                ...
                <div class="flex items-start gap-3">
                    <div class="flex-1 flex flex-col gap-1">
                        ...
                        {!! FormCustom::input('per', null, [ "placeholder" => '-', 'class' => 'number']) !!}
                    </div>
                    <div class="flex-2 flex flex-col gap-1">
                        <label class="text-sm font-normal text-gray-500">Pay*</label>
                        {!! FormCustom::input('pay', null, ['class' => 'number', "placeholder" => '-', 'prefixtext' =>
                        'Rp']) !!}
                    </div>
                </div>
            </section>
