...
<div class="flex flex-col gap-1">
    <div class="w-full h-9 flex items-center relative">
        ...
        <input name="{{ $name }}" value="{{ $value }}" type="{{ $atr->type }}"
            ...
            focus:shadow-xs/focused(4px-primary) focus:border-violet-300 {{ $atr->class }}"
            {{ $atr->required ? 'required' : '' }} {{ $atr->readonly ? 'readonly' : '' }}
