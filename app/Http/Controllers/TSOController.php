...
class TSOController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            if (request()->ajax()) {
                ...
                $order = null;
                $render =  view('Task.TSO.table', compact('employee_tso_datas', 'order', 'page_size'))->render();
    ...
    public function table_tso(Request $request)
    {
        ...
        $order = null;
        $render =  view('Task.TSO.table_tso', compact('employee_tso_datas', 'order', 'page_size'))->render();
        return $this->buildRes->RESPONSE_REQ('success', $render, null);
    }
    public function table_not_given_lb(Request $request)
    {
        ...
        $order = null;
        $render =  view('Task.TSO.table_not_given_lb', compact('order'))->render();
        return $this->buildRes->RESPONSE_REQ('success', $render, null);
    }
    public function approved_tso(Request $request)
    {
        ...
        try {
            $department_bios = $this->apiService->get_departments([]);
            $render = view('Task.TSO.approved_tso', compact('department_bios'))->render();
            return $this->buildRes->RESPONSE_REQ('success', $render, null);
        } catch (\Exception $e) {
            Log::emergency("File:" . $e->getFile() . "Line:" . $e->getLine() . "Message:" . $e->getMessage());
    ...
    public function approved_not_given_lb(Request $request)
    {
        ...
        try {
            $render = view('Task.TSO.approved_not_given_lb')->render();
            return $this->buildRes->RESPONSE_REQ('success', $render, null);
        } catch (\Exception $e) {
            Log::emergency("File:" . $e->getFile() . "Line:" . $e->getLine() . "Message:" . $e->getMessage());
        ...
        try {
            ...
            if ($validator->fails()) {
                ...
                $employee_tso = new EmployeeTso([
                    'tso_date' => Carbon::createFromFormat('d-m-Y', $reqdata['tso_date']),
    ...
    public function approved_not_given_lb_store(Request $request)
    {
        ...
        try {
            ...
            if ($validator->fails()) {
                ...
                $reqdata = $request->only(['emp_id', 'tso_date']);
