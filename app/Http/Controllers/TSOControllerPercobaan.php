...
class TSOControllerPercobaan extends Controller
{
    ...
    public function table_tso(Request $request)
    {
        ...
        $start_time = Carbon::parse("2022-10-18 23:59:59");
        $end_time = Carbon::parse("2022-10-18 23:59:59");
        ...
        $operational_time = Carbon::parse('2022-10-18')->format('Y-m-d');
        ...
        $operationals = $operationals->whereDate('date', $operational_time)->get();
        $employee_tsos = collect();
        $employee_lbs = collect();

        Log::info(response()->json($operationals));
        // Log::info($start_time_copy);
        // Log::info($start_time);
        foreach (($attens_groupings ?? []) as $emp_code => $attendance_employee) {
            ...
            // foreach ($attendance_employee as $key_attendance_emp_perdate => $itemAttendanceEmpPerDate) {
            //     $C_date = Carbon::parse($key_attendance_emp_perdate);
            //     $is_lessthan = $C_date->lt($start_time_copy);
            //     $is_greaterthan = $C_date->gt($start_time_copy);
            //     if (!$is_greaterthan) {
            //         $timetable = [];
            //         $timetable['id'] = '';
            //         $timetable['name'] = '';
            ...
            //                     $next_date_key = $dates[count($dates) - 1];
