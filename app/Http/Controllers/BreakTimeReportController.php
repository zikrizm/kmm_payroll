...
class BreakTimeReportController extends Controller
{
    private $service;
    private $buildRes;
        ...
        try {
            ...
            if (request()->ajax()) {
                ...
                if ($request->has('start_date') && $request->has('end_date')) {
                    ...
                    $render =  view('Report.break_time_report.table', compact('result', 'start_date_work_day', 'end_date_work_day', 'start_date_overtime', 'end_date_overtime'))->render();
                    return $this->buildRes->RESPONSE_REQ('success', $render, null);
                } else {
                    $render =  view('Report.break_time_report.table', compact('result'))->render();
                    return $this->buildRes->RESPONSE_REQ('success', $render, null);
                }
            }

            $department_bios = collect($this->service->get_departments(['page_size' => 999])['data']);
            return  view('Report.break_time_report.index', compact('department_bios',  'department_code', 'start_date', 'end_date'));
        } catch (\Exception $e) {
